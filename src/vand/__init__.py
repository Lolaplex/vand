#!/usr/bin/env python3
"""vand — catalog, pin, and replicate source instances."""
...
SKILL_NAME = "vand"
SKILL_PATHS_BEGIN = "<!-- vand-paths -->"
SKILL_PATHS_END = "<!-- /vand-paths -->"
...
def load_catalog() -> Catalog:
    if not CATALOG_PATH.exists():
        raise SystemExit(
            f"No catalog at {CATALOG_PATH}. Run: vand init --root <path>"
        )
...
def pip_editable_self() -> None:
    ...
    if result.returncode != 0:
        raise SystemExit("pip install -e failed; vand is not on PATH")


def infer_clone_root(root_arg: str) -> Path:
    """If --root is this checkout, use the parent so stacks land next to vand."""
    requested = Path(root_arg).expanduser().resolve()
    self_root = install_root()
    if requested == self_root:
        parent = self_root.parent
        print(f"This checkout is vand; clone root -> {parent}")
        return parent
    return requested
...
def format_self_check(result: SelfCheckResult, verbose: bool = True) -> str:
    ...
    lines = [
        f"vand install: {normalize_path(result.install_path)}",
    ...
    if result.status == "behind":
        ...
        lines.append(f"-> {detail}. Run: vand self-update")
...
def maybe_warn_self_update() -> None:
    if os.environ.get("VAND_SKIP_SELF_CHECK") == "1":
        return
    ...
    if result.status == "behind":
        ...
        print(
            f"\nNote: vand update available ({local} -> {remote}). "
            f"Run: vand self-check --fetch  |  vand self-update",
            file=sys.stderr,
        )
...
def apply_self_update(
    *,
    ...
    if not (root / ".git").exists():
        message = "vand is not a git checkout - clone from GitHub to self-update."
        if interactive:
    ...
    refuse = {
        ...
        "ahead": "Local vand is ahead of upstream - not pulling.",
        "no-local-git": "vand is not a git checkout - clone from GitHub to self-update.",
    }
    if result.status != "behind":
        message = refuse.get(
        ...
        if result.status in ("dirty", "diverged"):
            print(
                f"\nNote: vand {result.status} - skip self-update.",
                file=sys.stderr,
            )
        return result
...
def pin_hook_script(hook_name: str, invoke: str) -> str:
    lines = [
        "#!/bin/sh",
        f"# {PIN_HOOK_MARKER}",
        'LOG="${HOME:-$USERPROFILE}/.vand/logs/hook-pin.log"',
...
def consolidate_repo(
    ...
    *,
    ...
    if merge_in_progress(path) or rebase_in_progress(path):
        ...
        for f in conflicts[:10]:
            print(f"    {f}")
        print(f"  fix files, then: vand consolidate --continue {entry.name}")
    ...
    try:
        ...
        for f in conflicts[:10]:
            print(f"    {f}")
        print(f"  fix files, then: vand consolidate --continue {entry.name}")
        print(f"  or abort: vand consolidate --abort {entry.name}")
        return "conflict", [f"conflict {entry.name}"]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("self-check", help="Check whether vand itself is up to date")
    p.add_argument(
        "--fetch",
        action="store_true",
    ...
    p = sub.add_parser(
        "self-update",
        help="Fast-forward vand from descendant remotes and reinstall",
    )
    p.set_defaults(func=cmd_self_update)

    p = sub.add_parser(
        "install-skills",
        help="Copy the vand agent skill to ~/.cursor/skills and ~/.agents/skills",
    )
