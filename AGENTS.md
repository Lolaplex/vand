...
This is a Python 3 stdlib CLI. Catalog is `~/.git-updater/` (this machine). Shareable pins are `vendor.lock` (relative paths). Follow [`skills/git-updater/SKILL.md`](skills/git-updater/SKILL.md) for daily use. Flags: `git-updater --help-json`, never scrape `--help`.

## Procedure

1. Same interpreter Cursor uses. From the clone:
   - `python git_updater.py init --root <clone-root>`
   - Windows: `py -3` if that is what Cursor uses. macOS/Linux: `python3` if `python` is missing.
...
4. Tell the human **one** thing: reload Cursor so the user-level skill is picked up. You cannot do that for them.
